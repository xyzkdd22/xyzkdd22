...
from models import fixmatch3d
from models.simsiam3d import D
...
def train_step(args, model, labeled_trainloader, unlabeled_trainloader, optimizer, scheduler, writer, epoch):
    # Track logs
    losses = AverageMeter()
    ...
    for labeled_sample, unlabeled_sample in tqdm(zip(labeled_trainloader, unlabeled_trainloader), total=total):
        ...
        _, _, logits_x = model(inputs_x)
        z1, p1, logits_u_mask = model(inputs_u_mask)
        z2, p2, logits_u_bp = model(inputs_u_bp)
        ...
        # Compute total loss 
        loss = labeled_loss.mean() + args.mu * unlabeled_loss + args.beta * L
...
def main():
    ...
    # define the model
    encoder = get_models(args.arch, pretrained=False)
    model = fixmatch3d.ContrastiveFixMatch(encoder)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train_logs = train_step(args, model, labeledtrainloader, unlabeledtrainloader, optimizer, scheduler, writer, epoch)
